...
class Inference(object):
    ...
    def _initDataloader(self):
        ...
        return val_loader, val_salsa_loader
    ...
    def run(self):
        ...
        with torch.no_grad():
            t_start = time.time()
            for i, (proj_feature, input_label, _, proj_depth, uproj_x_idx, uproj_y_idx, uproj_depth) in enumerate(self.val_loader):
                
                uproj_x_idx = uproj_x_idx[0].cuda()
                uproj_y_idx = uproj_y_idx[0].cuda()
                uproj_depth = uproj_depth[0].cuda()
                proj_depth = proj_depth[0].cuda()
                proj_feature = proj_feature.cuda()
                input_label = input_label.long().cuda()

                # do post process
                t_process_start = time.time()
                pred_output = self.model(proj_feature)
                ...
                if self.settings.has_label:
                    _, sem_label_str, _ = self.salsa_loader.dataset.loadDataByIndex(
                            i)
                    sem_label = self.salsa_loader.dataset.labelMapping(
                        sem_label_str)
                ...
                lidar_token = self.salsa_loader.dataset.token_list[i]
                ...
                t_start = time.time()
                print("Iter [{:04d}|{:04d}] Datatime: {:0.3f} ProcessTime: {:0.3f}".format(
        ...
        for i in range(self.settings.n_classes):
            distribution_table.add_row(
                [self.salsa_loader.dataset.mapped_cls_name[i], dist_data[i].item()])
        self.recorder.logger.info("---- Data Distribution -----")
        self.recorder.logger.info(distribution_table)
...
class Experiment(object):
    ...
    def _initModel(self):
        model = pc_processor.models.SalsaNext(
            in_channel=5, nclasses=self.settings.n_classes)
        return model
